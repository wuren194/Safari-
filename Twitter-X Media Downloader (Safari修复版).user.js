// ==UserScript==
// @name         Twitter/X Media Downloader (Safari修复版)
// @name:zh-CN   Twitter 媒体下载 (Safari修复版)
// @namespace    https://github.com/ChinaGodMan/UserScripts
// @version      2025.12.24.safari
// @description  Download videos/pictures with one click | Safari compatible version using Blob download
// @description:zh-CN 一键下载视频/图片 | Safari兼容版，修复GM_download不工作的问题
// @author       Original: goemon2017,天音,Tiande,人民的勤务员 | Safari fix by Claude
// @license      MIT
...
/**
 * Safari修复版说明：
 * 1. 使用 GM_xmlhttpRequest + Blob 代替 GM_download
 * 2. 解决 Tampermonkey Safari 版 GM_download 假成功问题
 */
...
const TMD = (function () {
    ...
    // Safari兼容的下载函数
    function safariDownload(url, filename, onSuccess, onError) {
        GM_xmlhttpRequest({
            method: 'GET',
            url: url,
            responseType: 'blob',
            onload: function (response) {
                if (response.status === 200) {
                    try {
                        ...
                        if (onError) onError({ details: { current: e.message } });
                    }
                } else {
                    if (onError) onError({ details: { current: 'HTTP ' + response.status } });
                }
            },
            onerror: function (error) {
                if (onError) onError({ details: { current: error.statusText || 'Network Error' } });
            }
        });
    }
    ...
    return {
        ...
        selectTweetDialog: function (originalUser, quotedUser) {
            return new Promise((resolve) => {
                ...
                dialog.style.cssText = `
                    ...
                    box-shadow: 0 8px 32px rgba(0, 0, 0, 0.3); font-family: system-ui, -apple-system, sans-serif;
                `
                const title = document.createElement('h3')
                title.textContent = `${lang.choose}`
                title.style.cssText = `margin-top: 0; margin-bottom: 20px; text-align: center; color: #0f1419;`
                ...
                originalBtn.style.cssText = `
                    background: #1DA1F2; color: white; border: none; border-radius: 50px;
                    padding: 16px; font-size: 16px; font-weight: 600; cursor: pointer; transition: background 0.2s;
                `
                ...
                originalBtn.addEventListener('click', () => { resolve('original'); document.body.removeChild(overlay) })

                const quotedBtn = document.createElement('button')
                quotedBtn.textContent = `${lang.quote} (by ${quotedUser})`
                quotedBtn.style.cssText = `
                    background: #fff; color: #1DA1F2; border: 2px solid #1DA1F2; border-radius: 50px;
                    padding: 16px; font-size: 16px; font-weight: 600; cursor: pointer; transition: all 0.2s;
                `
                ...
                quotedBtn.addEventListener('click', () => { resolve('quoted'); document.body.removeChild(overlay) })

                const cancelBtn = document.createElement('button')
                cancelBtn.textContent = `${lang.cancel}`
                cancelBtn.style.cssText = `background: transparent; color: #657786; border: none; padding: 12px; font-size: 14px; cursor: pointer; margin-top: 8px;`
                cancelBtn.addEventListener('click', () => { resolve(null); document.body.removeChild(overlay) })
                ...
                overlay.addEventListener('click', (e) => {
                    if (e.target === overlay) { resolve(null); document.body.removeChild(overlay) }
                })
            })
        },
        ...
        click: async function (btn, status_id, is_exist, index) {
            ...
            let hasQuotedMedia = json.quoted_status_result?.result?.legacy?.media || json.quoted_status_result?.result?.legacy?.extended_entities?.media
            let tweet, user
            ...
            if (hasQuotedMedia) {
                ...
                if (!choice) { this.status(btn, 'download', lang.download); return }
                if (choice === 'quoted') {
            ...
            return {
                    ...
                    if (taskList.length > 1) {
                        ...
                        if (enable_packaging) {
                                ...
                                GM_xmlhttpRequest({
                                    ...
                                    onload: (response) => {
                                        if (response.status === 200) {
                                            ...
                                            tasks = tasks.filter(t => t.url !== task.url)
                                            thread--
                                            this.status(btn, 'failed', 'HTTP ' + response.status)
                                            this.update()
                                        }
                                    },
                                    onerror: (error) => {
                                        ...
                                        tasks = tasks.filter(t => t.url !== task.url)
                                        thread--
                                        this.status(btn, 'failed', error.statusText || 'Network Error')
                                        this.update()
                                    }
                                })
                            })
                            ...
                            taskList.forEach((task) => {
                                thread++
                                this.update()
                                safariDownload(task.url, task.name,
                                    () => {
                                        thread--
                                        tasks = tasks.filter(t => t.url !== task.url)
                                        this.status(btn, 'completed', lang.completed)
                        ...
                        thread++
                        this.update()
                        safariDownload(taskList[0].url, taskList[0].name,
                            () => {
                                thread--
                ...
                update: function () {
                    if (!notifier) {
                        ...
                        notifier.title = 'Twitter Media Downloader (Safari)'
        ...
        settings: async function () {
            ...
            let title = $element(dialog, 'h3', 'margin: 10px 20px;', lang.dialog.title + ' (Safari版)')
            ...
            show_enable_packaging_input.onchange = () => { GM_setValue('enable_packaging', show_enable_packaging_input.checked) }
        ...
        language: {
            en: {
                ...
                settings: 'Settings',
                dialog: { title: 'Download Settings', save: 'Save', save_history: 'Remember download history', clear_history: '(Clear)', clear_confirm: 'Clear download history?', show_sensitive: 'Always show sensitive content', pattern: 'File Name Pattern' },
            ...
            ja: {
                ...
                completed: 'ダウンロード完了',
                settings: '設定',
                dialog: { title: 'ダウンロード設定', save: '保存', save_history: 'ダウンロード履歴を保存する', clear_history: '(クリア)', clear_confirm: 'ダウンロード履歴を削除する？', show_sensitive: 'センシティブな内容を常に表示する', pattern: 'ファイル名パターン' },
            ...
            zh: {
                ...
                completed: '下载完成',
                settings: '设置',
                dialog: { title: '下载设置', save: '保存', save_history: '保存下载记录', clear_history: '(清除)', clear_confirm: '确认要清除下载记录？', show_sensitive: '自动显示敏感的内容', pattern: '文件名格式' },
            ...
            'zh-Hant': {
                ...
                completed: '下載完成',
                settings: '設置',
                dialog: { title: '下載設置', save: '保存', save_history: '保存下載記錄', clear_history: '(清除)', clear_confirm: '確認要清除下載記錄？', show_sensitive: '自動顯示敏感的内容', pattern: '文件名規則' },
                enable_packaging: '多文件打包成 ZIP',
                original: '原始推文',
                quote: '引用推文',
                cancel: '取消',
