...
// @version      23.0
// @description  【播放修复】修复视频无法播放的问题；优先使用原始视频链接，不再强制转换格式；增加自动格式试错功能。
...
(function () {
    ...
    const StyleManager = {
        CSS: `
            ...
            #lpsg-button-container { position: fixed !important; right: 24px !important; bottom: 100px !important; display: flex !important; flex-direction: column !important; gap: 16px !important; z-index: ${CONFIG.Z_LAYERS.BUTTON} !important; }
    ...
    const Core = {
        ...
        async downloadBatch() {
            ...
            for (let i = 0; i < targets.length; i++) {
                ...
                txt.innerText = `下载中 ${i + 1} / ${targets.length}`;
                try {
                    ...
                    a.download = `${title}-${String(seq).padStart(3, '0')}.${ext}`;
        ...
        updateVirtualDOM(force = false) {
            ...
            else if (STATE.currentTab === 'link') cols = 2;
    ...
    const HelperCore = {
        ...
        applyFlowSettings() {
            ...
            const hideTextPost = localStorage.getItem('LPSG_HideTextPost') === 'true';
            ...
            if (hideTextPost) {
                ...
                document.querySelectorAll('article.message').forEach(article => {
                    const hasMedia = article.querySelector('.message-content video, .message-content iframe, .message-content img:not(.smilie)');
                    article.style.display = hasMedia ? '' : 'none';
                    if(hasMedia) hasContent = true;
                });
                if (!hasContent && localStorage.getItem('LPSG_AutoJump') === 'true') {
                    setTimeout(() => {
                        const nextBtn = document.querySelector(localStorage.getItem('LPSG_JumpBackward') === 'true' ? 'a.pageNav-jump--prev' : 'a.pageNav-jump--next');
                        if (nextBtn) nextBtn.click();
                    }, 1000);
                }
            } else {
                document.querySelectorAll('article.message').forEach(a => a.style.display = '');
            }
        ...
        initUI() {
            ...
            modal.innerHTML = `
                ...
                <div class="${C.CONTENT}">
                    ...
                    <div class="${C.TAB_PANE}" id="tab-flow"><div class="${C.SETTING_ITEM}"><span class="${C.SETTING_LABEL}">隐藏用户信息</span><div class="${C.SWITCH}" id="sw-hide-user"></div></div><div class="${C.SETTING_ITEM}"><span class="${C.SETTING_LABEL}">点击折叠帖子</span><div class="${C.SWITCH}" id="sw-fold"></div></div><div class="${C.SETTING_ITEM}"><span class="${C.SETTING_LABEL}">隐藏纯文字帖</span><div class="${C.SWITCH}" id="sw-hide-text"></div></div><div class="${C.SETTING_ITEM}"><span class="${C.SETTING_LABEL}">自动跳过空页</span><div class="${C.SWITCH}" id="sw-auto-jump"></div></div><div class="${C.SETTING_ITEM}"><span class="${C.SETTING_LABEL}">向前翻页模式</span><div class="${C.SWITCH}" id="sw-jump-back"></div></div></div>
                </div>`;
        ...
        bindEvents() {
            ...
            bindSwitch('sw-hide-text', 'LPSG_HideTextPost', HelperCore.applyFlowSettings);
            bindSwitch('sw-auto-jump', 'LPSG_AutoJump', () => {});
            bindSwitch('sw-jump-back', 'LPSG_JumpBackward', () => {});
